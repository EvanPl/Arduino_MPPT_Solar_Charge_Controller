...
* Then, the circuit decides on which sub state to enter (*mode_select* function). There are five different sub states given below:
   ...
   4. ***bulk***. This is the first of the two charging sub states and the one where the the MPPT algorithm, **Perturb and observe**, takes place. The purpose of this algorithm (whose flowchart is shown in the left figure below) is to continuously change the PWM signal applied to the MOSFET of the buck converter and lock to the one which provides the largest output solar power (which is also being tracked continuously). The figure on the right, below, shows a typical Power vs. Voltage curve of a solar panel where the maximum power point is at its peak (between points 2 and 3). The bulk sub state occurs when there is sufficient sunlight and the battery voltage is between 1.6V and 2V (float voltage). Last but not least, the initial PWM signal used in the Perturb and Observe algorithm is 30%.
